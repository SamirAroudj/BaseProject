...
namespace Graphics
{
	...
	public:
		...
		@param aspectRatio This is the image width to height ratio of the camera. Must be positive.*/
		...
		virtual Math::Matrix3x3 computeHPSToNNRayDirWS(const Utilities::ImgSize &viewportSize, const bool addPixelCenterOffset = true) const = 0;
		...
		virtual Math::Matrix4x4 computeWorldSpaceToPixelSpaceMatrix(const Utilities::ImgSize &viewportSize, const bool considerPixelCenterOffset = true) const = 0;
		...
		/** Sets the aspect ratio of the camera (always width / height).
		@param aspectRatio Set this to the aspect ratio (width / height) you want to store. Must be positive. */
