...
namespace Graphics
{
	...
	public:
		...
		@param principalPoint Defines which normalized coordinates (x, y) map to the viewport center. (Defines where the principal axis of the camera hits the camera plane.)
			Set this to pp = (0.5, 0.5) = (half viewport width, half viewport height) for having normalized device coordinates at (0, 0) mapped to the center of the normalized device cube.
		...
		@param principalPoint Defines which normalized coordinates (x, y) map to the viewport center. (Defines where the principal axis of the camera hits the camera plane.)
			Set this to pp = (0.5, 0.5) = (half viewport width, half viewport height) for having normalized device coordinates at (0, 0) corresponding to the center of the normalized device cube.
		@param sizeOfNDCCube Defines the size of the normalized device coordinates cube.
		...
		/** Transforms a 2D position in normalized device coordinates (NDC) to a position in pixel coordinates (PC).
			...
			Assumes a principal point of (0.5, 0.5).
		...
		@param resolution Set this to the resolution of the window you want to get pixel coordinates for.
		@param addPixelCenterOffset (0.5, 0.5) is added at the end if this is set to true so that pixel centers are returned instead of corner coordinates.
		@param principalPoint Set this to the principal point which defines the center of the image / where the ray x=0, y=0 is mapped to. Normally this is (0.5, 0.5).
		...
		static Math::Vector2 transformNDCToPC(const Math::Vector2 &pNDC, const Utilities::ImgSize &resolution, const bool addPixelCenterOffset, const Math::Vector2 &principalPoint, const Real sizeOfNDCCube);
